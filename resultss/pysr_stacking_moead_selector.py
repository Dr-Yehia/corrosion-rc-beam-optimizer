...
try:
    ...
    class _StdLoggerAdapter:
        def __init__(self) -> None:
            self._logger = logging.getLogger("pysr_stacking_selector")
        ...
        def add(self, sink, level="INFO", **_: object) -> None:
            if sink == sys.stderr:
                return None
            try:
                ...
                self._logger.warning("Could not attach file logger %s: %s", sink, exc)
...
def build_symbolic_inputs(
    ...
    d_col = find_column(df, "Depth (mm)", "Depth", "depth")
    b_col = find_column(df, "Width (mm)", "Width", "width")
    db_col = find_column(df, "Diameter Tensile Bars", "db,t", "db_t")
    fy_col = find_column(df, "fy Longitudinal Bars", "fy")
    fc_col = find_column(df, "f'c (MPa)", "f'c", "fc")
...
def run_pysr(
    ...
    search_modes = [
        (
            "fast_relative_mt",
            {
                "elementwise_loss": relative_loss,
                "deterministic": False,
    ...
    for mode_name, extra_kwargs in search_modes:
        try:
            logger.info(f"Trying PySR mode: {mode_name}")
            model = PySRRegressor(**common_kwargs, **extra_kwargs)
            model.fit(x_np, y_train, variable_names=variable_names)

            eq_df = getattr(model, "equations_", None)
            if eq_df is None:
                raise RuntimeError("PySR finished without equations_ output.")
            if not isinstance(eq_df, pd.DataFrame):
                eq_df = pd.DataFrame(eq_df)
            if eq_df.empty:
                raise RuntimeError("PySR produced an empty hall-of-fame table.")

            logger.info(f"PySR succeeded in mode '{mode_name}' with {len(eq_df)} equations.")
            return model, eq_df.copy(), mode_name
        except Exception as exc:
            last_exc = exc
            logger.warning(f"PySR mode '{mode_name}' failed: {exc}")

    raise RuntimeError("PySR failed in all compatibility modes.") from last_exc
...
def select_rankable_indices(candidates: List[Candidate]) -> Tuple[List[int], str]:
    ...
    finite_only = [
        idx
        for idx, candidate in enumerate(candidates)
        if "non_finite_metric" not in candidate.failures
    ]
    if finite_only:
        logger.warning("No candidate passed relaxed gates; falling back to finite-metric candidates.")
        return finite_only, "finite_only"
...
def save_outputs(
    ...
    (EQ_DIR / "best_equation_stacking.txt").write_text(
        ...
        f"# Search mode={search_mode} | Selection mode={selection_mode} | rankable={len(rankable_idx)} "
