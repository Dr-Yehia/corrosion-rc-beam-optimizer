...
def save_outputs(
    ...
    z_pred  = np.clip(np.nan_to_num(z_pred, nan=0.0, posinf=1.5, neginf=-1.5), -1.5, 1.5)
    ...
    if y_true_test is not None and data_dict_test is not None and M0_test is not None:
        ...
        z_t = np.clip(np.nan_to_num(z_t, nan=0.0, posinf=1.5, neginf=-1.5), -1.5, 1.5)
...
def _refit_constants(eq_str, data_dict, y_true, M0):
    """Refit constants via differential_evolution + Nelder-Mead polish."""
    from scipy.optimize import differential_evolution, minimize
    try:
        expr_sp = safe_sympify(eq_str)
        consts = sorted([a for a in expr_sp.atoms(sp.Number) if abs(float(a)) > 1e-12], key=float)
        if not consts:
            return eq_str
        ...
        fn = sp.lambdify(list(data_dict.keys()) + [str(s) for s in symbols_c], expr_sub, modules=["numpy"])
        fv = [data_dict[k] for k in data_dict.keys()]
        x0 = np.array([float(c) for c in consts])
        def loss(x):
            try:
                z = np.clip(np.nan_to_num(np.asarray(fn(*fv, *x), dtype=float), nan=0, posinf=1.5, neginf=-1.5), -1.5, 1.5)
                return float(np.mean(np.abs((y_true - M0 * np.exp(z)) / np.maximum(y_true, 1e-9))))
            except Exception:
                return 1.0
        mape_before = loss(x0) * 100
        bounds = [(v*0.3, v*3.0) if abs(v) > 0.01 else (v-0.5, v+0.5) for v in x0]
        try:
            best_x = differential_evolution(loss, bounds, seed=42, maxiter=300, tol=1e-8, polish=True, init='sobol').x
        except Exception:
            best_x = x0
        res = minimize(loss, best_x, method="Nelder-Mead", options={"maxiter": 15000, "xatol": 1e-8, "fatol": 1e-8})
        if res.fun < loss(best_x):
            best_x = res.x
        expr_opt = expr_sub
        for sym, val in zip(symbols_c, best_x):
            expr_opt = expr_opt.subs(sym, sp.Float(round(float(val), 6)))
        logger.info(f"Refit: MAPE {mape_before:.2f}% -> {loss(best_x)*100:.2f}% ({len(consts)} consts)")
        return str(expr_opt)
    except Exception as exc:
        logger.warning(f"Refit failed: {exc}")
        return eq_str


def _refit_all_candidates(cands, data_dict, y_true, M0, top_k=30):
    """Refit top-K candidates' constants and return best by MAPE."""
    logger.info(f"EXHAUSTIVE REFIT: optimizing top {top_k} candidates")
    scored = sorted(enumerate(cands), key=lambda x: x[1].metrics.get("MAPE", 999))[:top_k]
    best_mape, best_idx = float("inf"), scored[0][0]
    for rank, (idx, c) in enumerate(scored):
        eq_r = _refit_constants(c.equation, data_dict, y_true, M0)
        try:
            expr_sp = safe_sympify(eq_r)
            fn = sp.lambdify(tuple(data_dict.keys()), expr_sp, modules=["numpy"])
            z = np.clip(np.nan_to_num(np.asarray(fn(*[data_dict[k] for k in data_dict]), dtype=float), nan=0, posinf=1.5, neginf=-1.5), -1.5, 1.5)
            yp = M0 * np.exp(z)
            mape = float(np.mean(np.abs((y_true - yp) / np.maximum(y_true, 1e-9))) * 100)
            r2 = float(r2_score(y_true, yp))
        except Exception:
            continue
        logger.info(f"  [{rank+1}/{len(scored)}] C={c.complexity:.0f} MAPE={mape:.2f}% R2={r2:.4f}")
        cands[idx] = Candidate(eq_r, c.complexity, {**c.metrics, "MAPE": round(mape,2), "R2": round(r2,4)}, c.objectives, mape)
        if mape < best_mape:
            best_mape, best_idx = mape, idx
    logger.success(f"BEST AFTER REFIT: idx={best_idx}, MAPE={best_mape:.2f}%")
    return cands, best_idx


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="PG-RSR: Physics-Guided Residual Symbolic Regression")
...
def main() -> None:
    ...
    # ── DISTILLATION TARGET: smooth stacking + experimental blend ───────────
    z_exp   = np.log(np.maximum(y_true,  1e-9) / M0)
    z_stack = np.log(np.maximum(m_stack, 1e-9) / M0)
    ALPHA = 0.80  # 80% stacking (smooth) + 20% experimental (anchor)
    y_target = np.clip(ALPHA * z_stack + (1 - ALPHA) * z_exp, -1.5, 1.5)
    logger.info(
        f"DISTILLATION target (alpha={ALPHA}): mean={y_target.mean():.3f}, "
        f"std={y_target.std():.3f}, range=[{y_target.min():.3f}, {y_target.max():.3f}]"
    )

    # PySR trains on 60% train only
    X_sym_train    = X_sym.iloc[train_idx].reset_index(drop=True)
    y_target_train = y_target[train_idx]

    # Validation set — evaluate against REAL experimental data
    data_dict_val = {k: v[val_idx] for k, v in data_dict.items()}
    y_true_val    = y_true[val_idx]
    M0_val        = M0[val_idx]
    z_exp_val     = z_exp[val_idx]
    ...
    # Evaluate on VALIDATION set against REAL data
    cands = evaluate_candidates(eq_df, data_dict_val, y_true_val, z_exp_val, M0_val)

    if not cands:
        raise RuntimeError("No valid candidate equations. Try increasing --niterations.")

    # ── EXHAUSTIVE REFIT: DE+NM on top 30 candidates, pick best MAPE ─────
    data_dict_trainval = {k: v[trainval_idx] for k, v in data_dict.items()}
    cands, best_idx = _refit_all_candidates(
        cands, data_dict_trainval, y_true[trainval_idx], M0[trainval_idx], top_k=30,
    )
