...
"""
═══════════════════════════════════════════════════════════════
  Corrosion RC Beam Optimizer — v5 (Ratio-Fix + Full Figures)
  Google Colab Self-Contained Script
═══════════════════════════════════════════════════════════════
  FIXES:
    1. PySR target  → Ratio = Mmax,exp / M_ACI  (not raw Mmax)
    2. PySR settings → niterations=500, populations=80, maxsize=20
    3. 8 publication-quality figures generated
    4. Statistical validation recalculated
    5. Cohen's d expected to improve to large effect
...
# ════════════════════════════════════════════════════════════
# CELL 8: PySR — RATIO TARGET (THE KEY FIX)
# ════════════════════════════════════════════════════════════
logger.info("\n" + "=" * 60)
logger.info(" Phase 3 — PySR Symbolic Regression (RATIO TARGET)")
logger.info("=" * 60)
...
# Key insight: PySR will find f_corr such that Mmax = M_ACI * f_corr
# This removes the geometric complexity and lets PySR focus on corrosion effect

# Features for Ratio model: remove d and b (already in M_ACI)
RATIO_FEATURES = [
...
# Also remove extreme outliers (ratio > 10 or < 0.1)
...
# --- Configure PySR with stronger settings ---
pysr_model = PySRRegressor(
    niterations      = 500,       # was 200
    maxsize          = 20,        # was 25, smaller = simpler equations
    populations      = 80,        # was 40
    binary_operators = ["+", "-", "*", "/", "^"],
...
# --- Evaluate the Ratio equation ---
ratio_pred = pysr_model.predict(X_pysr)
...
# Back-transform to Mmax = M_ACI * f_corr
...
best_eq_str   = str(pysr_model.sympy())
best_eq_latex = str(pysr_model.latex())
...
logger.info("=" * 50)
logger.info(" PySR RATIO Equation — Results")
logger.info("=" * 50)
logger.info(f"  f_corr = {best_eq_str}")
...
with open(EQ_DIR / "best_equation.txt", "w") as f:
    f.write(f"# Best PySR Equation (Ratio Approach)\n")
...
with open(EQ_DIR / "best_equation.latex", "w") as f:
    f.write(f"% Best PySR Equation — LaTeX (Ratio Approach)\n")
    f.write(f"% Generated: {datetime.now()}\n\n")
    f.write(f"M_{{\\max,corr}} = M_{{\\text{{ACI}}}} \\times {best_eq_latex}\n")

# Get all equations from Hall of Fame
...
# ════════════════════════════════════════════════════════════
# CELL 9: GENERATE 8 PUBLICATION FIGURES
# ════════════════════════════════════════════════════════════
logger.info("\n" + "=" * 60)
...
# ── Figure 1: Predicted vs Experimental (CatBoost) ──
...
ax1.set_title(f"CatBoost: Predicted vs Experimental\nR² = {r2_score(y_true, y_model):.4f}")
...
# ── Figure 2: Predicted vs Experimental (Ratio Equation) ──
...
if "complexity" in eq_df.columns and "loss" in eq_df.columns:
    ...
    ax3.scatter(eq_df.loc[best_idx, "complexity"], eq_df.loc[best_idx, "loss"],
               s=200, c="red", zorder=5, marker="*", label="Best (selected)")
...
ax5.hist(errors_model, bins=40, alpha=0.7, color="#1565C0", label=f"CatBoost (σ={np.std(errors_model):.2f})", density=True)
ax5.hist(errors_aci, bins=40, alpha=0.5, color="#E65100", label=f"ACI 318-19 (σ={np.std(errors_aci):.2f})", density=True)
ax5.axvline(x=0, color='red', linestyle='--', linewidth=1.5)
ax5.set_xlabel("Prediction Error (kN·m)")
ax5.set_ylabel("Density")
ax5.set_title("Error Distribution: CatBoost vs ACI 318-19")
...
ax7.plot(eta_vals[sort_idx], ratio_pred[sort_idx], c="#D32F2F", linewidth=0.5, alpha=0.5)
...
# ── Figure 8: Taylor Diagram (simplified) ──
fig8, ax8 = plt.subplots(figsize=(8, 8))
def _taylor_stats(obs, pred):
    """Return std_pred, correlation, RMSE_centered."""
    std_o = np.std(obs)
    std_p = np.std(pred)
...
models_taylor = {
    ...
    "CatBoost": (y_true, y_model),
}
if len(y_mmax_valid) == len(mmax_pred_from_ratio):
    # Can only add if same samples — use matching indices
    pass  # Skip adding ratio equation to Taylor for simplicity

colors_t = {"ACI 318-19": "#E65100", "CatBoost": "#1565C0"}
markers_t = {"ACI 318-19": "s", "CatBoost": "^"}
...
for name, (obs, pred) in models_taylor.items():
    ...
    x = std_r * corr
    y = std_r * np.sqrt(1 - corr**2)
    ax8.scatter(x, y, s=150, c=colors_t[name], marker=markers_t[name],
...
sep = "=" * 65
print(f"\n{sep}")
print(" CORROSION RC BEAM OPTIMIZER v5 — PIPELINE COMPLETE")
print(sep)
print(f"\n  ACI 318-19 Baseline:")
...
print(f"\n  📊 Figures generated: 8 (in {FIGURES_DIR})")
