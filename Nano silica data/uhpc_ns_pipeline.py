...
def _pipeline(X_raw, y_raw, n_trials, M0_all=None):
    ...
    # KNN Imputation (fit on clean train only)
    ...
    # [★] Predict and back-transform to y-space
...
def _ns_curve(model, Xtr_raw, ns_i, imp, sc, prop, unit, out, M0_med=None):
    ...
    rng = np.linspace(0, 200, 200); pred = []
    for v in rng:
        x  = med.copy(); x[ns_i] = v
...
def _shap_interaction(sv, Xte, feats, ns_col, sf_col, prop, out):
    ...
    ax.set_ylabel("SHAP value for NS (log-ratio correction)")
...
def run_property(cfg, df, ns_col, sf_col):
    ...
    df_t = df_t.loc[:, [c for c in df_t.columns
    ...
    # Train/test split for target encoding (same seed → same split as _pipeline)
    ...
    # All figures — use pre-computed y-space predictions
    ...
    opt_ns = _ns_curve(
        ...
        art["imp"], art["sc"], cfg["name"], cfg["unit"], out,
        M0_med=art["M0_tr_median"])

    _taylor(art["yte_r"], art["y_preds"], cfg["name"], out)
